    ...
    public boolean doPostContinue(RegistrationStep step, RegistrationContext registrationContext)
        ...
        if (Constants.COMPLETE.equals(step.getFlowStatus())) {
            ...
            try {
                ...
                loggerInputs.put("flowId", registrationContext.getContextIdentifier());
                loggerInputs.put("userName", maskIfRequired(user.getUsername()));
                loggerInputs.put("notificationChannel", preferredChannel);
            ...
            } catch (IdentityEventException | IdentityRecoveryException e) {
