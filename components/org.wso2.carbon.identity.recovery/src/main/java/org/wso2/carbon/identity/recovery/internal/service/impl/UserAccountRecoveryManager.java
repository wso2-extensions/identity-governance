        ...
        if (!expressionConditionList.isEmpty()) {
            ...
            if (userList.size() > 1) {
                ...
                for (org.wso2.carbon.user.core.common.User user : userList) {
                    ...
                    try {
                        usernameRecoveryManager.triggerNotification(recoveryUser,
                                NotificationChannels.EMAIL_CHANNEL.getChannelType(),
                                IdentityEventConstants.Event.TRIGGER_NOTIFICATION, null);
                    }catch (IdentityRecoveryException e) {
                        log.warn("Error while triggering notification for the user: " +
                                user.getDomainQualifiedUsername(), e);
                    }
                }
                throw Utils.handleClientException(
        ...
        for (Map.Entry<String,String> entry : claims.entrySet()) {
            ...
            if (StringUtils.isNotEmpty(entry.getKey()) && StringUtils.isNotEmpty(entry.getValue()) &&
                ...
                if (!(Boolean.parseBoolean(IdentityUtil.getProperty(
                        IdentityRecoveryConstants.ConnectorConfig.USERNAME_RECOVERY_SHARED_EMAIL)) && attributeName
                        .equals(IdentityRecoveryConstants.GIVEN_NAME))) {
