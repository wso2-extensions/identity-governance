...
public class UserSelfRegistrationManager {
  ...
  public NotificationResponseBean registerUser(User user, String password, Claim[] claims,
      Property[] properties) throws IdentityRecoveryException {
    String[] roles = new String[0];
    return registerUser(user, password, roles, claims, properties);
  }

  public NotificationResponseBean registerUser(User user, String password, String[] roles,
      Claim[] claims, Property[] properties) throws IdentityRecoveryException {
    ...
    if (!enable) {
      throw Utils.handleClientException(
          IdentityRecoveryConstants.ErrorMessages.ERROR_CODE_DISABLE_SELF_SIGN_UP,
          user.getUserName());
    }
    ...
    try {
      ...
      try {
        userStoreManager = realmService.getTenantUserRealm(
            IdentityTenantUtil.getTenantId(user.getTenantDomain())).getUserStoreManager();
      } catch (UserStoreException e) {
        throw Utils.handleServerException(
            IdentityRecoveryConstants.ErrorMessages.ERROR_CODE_UNEXPECTED, user.getUserName(), e);
      }
      ...
      try {
        ...
        // TODO It is required to add this role before tenant creation.
        // And also, this role should not not be able remove.
        ...
        userStoreManager.addUser(
            IdentityUtil.addDomainToName(user.getUserName(), user.getUserStoreDomain()), password,
        ...
        if (e.getMessage() != null && e.getMessage().contains("UserAlreadyExisting:")) {
          ...
          throw Utils.handleServerException(
              IdentityRecoveryConstants.ErrorMessages.ERROR_CODE_ADD_SELF_USER, user.getUserName(),
              e);
        }
      }
      ...
      // isNotificationInternallyManage == true, will be handled in
      // UserSelfRegistration Handler
  ...
  public void confirmUserSelfRegistration(String code) throws IdentityRecoveryException {
    ...
    // if return data from load method, it means the code is validated.
    // Otherwise it returns exceptions
    ...
    try {
      ...
      try {
        ...
        throw Utils.handleServerException(
            IdentityRecoveryConstants.ErrorMessages.ERROR_CODE_UNLOCK_USER_USER,
            user.getUserName(), e);
      }
  ...
  public NotificationResponseBean resendConfirmationCode(User user, Property[] properties)
    ...
    if (userRecoveryData == null || StringUtils.isBlank(userRecoveryData.getSecret())
      ...
      throw Utils.handleClientException(
          IdentityRecoveryConstants.ErrorMessages.ERROR_CODE_OLD_CODE_NOT_FOUND, null);
    }
    // Invalid old code
    ...
    if (isNotificationInternallyManage) {
      triggerNotification(user,
          IdentityRecoveryConstants.NOTIFICATION_TYPE_RESEND_ACCOUNT_CONFIRM.toString(), secretKey,
          properties);
    } else {
      notificationResponseBean.setKey(secretKey);
    }
  ...
  private void triggerNotification(User user, String type, String code, Property[] props)
      throws IdentityRecoveryException {
