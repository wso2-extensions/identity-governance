...
public class UserSelfRegistrationManager {
	...
	public NotificationResponseBean registerUser(User user, String password, String[] roles, Claim[] claims,
		...
		try {
			...
			try {
				userStoreManager.setUserClaimValues(
						IdentityUtil.addDomainToName(user.getUserName(), user.getUserStoreDomain()), userClaims, null);
			} catch (UserStoreException e) {
