...
public class SecurityQuestionsApi  {
   ...
   private final SecurityQuestionsApiService delegate = SecurityQuestionsApiServiceFactory.getSecurityQuestionsApi();
    ...
    @io.swagger.annotations.ApiOperation(value = "",
            notes = "This API is used to initiate password recovery using user challenge questions at once. " +
                    "Response will be a random challenge questions with a confirmation key.\n",
    ...
    public Response securityQuestionsGet(
            @ApiParam(value = "username of the user", required = true) @QueryParam("username") String username,
            @ApiParam(value = "`User Store Domain` which user belongs. If not specified, " +
                              "it will be `PRIMARY` domain.\n")
            @QueryParam("realm") String realm,
            @ApiParam(value = "`Tenant Domain` which user belongs. If not specified, " +
                              "it will be `carbon.super` domain.\n")
